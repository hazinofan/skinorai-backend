...
import { ProductQueryDto, SensitivityLevel } from './dto/product-query.dto';
...
export class ProductsService implements OnModuleInit {
  ...
  async listProducts(query: ProductQueryDto) {
    ...
    if (query.productType && query.productType !== 'all') {
      products = products.filter((product) => product.productType === query.productType);
    }
    ...
    const sorted = scored.sort((left, right) => {
      ...
      if (query.sort === 'newest') return right.createdAt.getTime() - left.createdAt.getTime();
      if (hasRecommendationInput || query.sort === 'recommended' || query.sort === 'score') {
    ...
    const items = sorted.slice(start, start + limit).map((product) => this.toResponse(product));
  ...
  async getProduct(slug: string) {
    const product = await this.productsRepository.findOneBy({ slug, isActive: true });
  ...
  private matchesHardFilters(product: ScoredProduct, query: ProductQueryDto) {
    if (query.skinType && query.skinType !== 'all' && !product.skinTypes.includes(query.skinType)) {
      return false;
    }

    if (query.goal && query.goal !== 'all' && !product.goals.includes(query.goal)) {
      return false;
    }
  ...
  private scoreProduct(product: ProductEntity, query: ProductQueryDto): ScoredProduct {
    let score = 55;
    const reasons: string[] = [];
    const warnings: string[] = [];
    const avoidIngredients = this.parseList(query.avoidIngredients).map((item) => item.toLowerCase());
    ...
    if (query.sensitivity && query.sensitivity !== 'all') {
      const level = query.sensitivity as SensitivityLevel;
      const isSensitiveProduct = product.skinTypes.includes('sensitive') || product.tags.includes('sans parfum');
    ...
    if (avoidIngredients.length > 0) {
      const productWatchouts = product.watchoutIngredients.map((item) => item.toLowerCase());
      const productKeys = product.keyIngredients.map((item) => item.toLowerCase());
      const matchedAvoid = avoidIngredients.filter((avoid) =>
        [...productWatchouts, ...productKeys].some((ingredient) => ingredient.includes(avoid)),
      );
      if (matchedAvoid.length > 0) {
        score -= 30;
        warnings.push(`Contient un élément à éviter: ${matchedAvoid.join(', ')}`);
      } else {
        score += 8;
